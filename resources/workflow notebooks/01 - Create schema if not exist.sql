...
-- MAGIC
-- MAGIC  > Note that here we're using the syntax `${parameter_name}`. Technically this syntax for parameters is deprecated on Databricks Run time (DBR) 15+ and you will likely receive a warning. The correct syntax for DBR 15+ should be `IDENTIFIER(:parameter_name)` however for the `ALTER TABLE table OWNER TO` command the new syntax doesn't work as intended. Thankfully the deprecated syntax does still work on DBR 15+ so that is why we are using it here.
